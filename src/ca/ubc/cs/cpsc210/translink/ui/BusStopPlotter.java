    ...
    public void markStops(Location currentLocation) {
        ...
        for (Stop stop: StopManager.getInstance()) {
            ...
            if (Geometry.rectangleContainsPoint(northWest, southEast, stopLocn)) {
                ...
                GeoPoint geo = new GeoPoint(stopLocn.getLatitude(), stopLocn.getLongitude());

                m.setPosition(geo);
                m.setRelatedObject(stop);
                m.setTitle((stop.getNumber() + " " + stop.getName()) + stopTitle(stop));
        ...
        updateMarkerOfNearest(newStop);

    }

    // EFFECTS: Add the route numbers to the stop title line by line
    public String stopTitle(Stop stop) {
        String routes = "";
        for (Route route : stop.getRoutes()) {
            routes = routes + System.lineSeparator() + route.getNumber();
        }
        return routes;
    }
